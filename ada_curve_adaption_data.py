...
def eval_model_vs_hilbert(model:tf.keras.Model, x_tf:tf.Tensor, grid:np.ndarray, qp_matrix:np.ndarray) -> None:
    ...
    hilbert_idxs = encode(np.floor(grid).astype(np.int32), num_dims=N_DIMS, num_bits = N_BITS) 
    hilbert_ranks = np.argsort(hilbert_idxs, kind = "stable")
    
    np.savetxt("data_points.csv", x_tf.numpy(), delimiter=",")
    np.savetxt("model_ranks.csv", model_embeddings.numpy(), delimiter=",")
    np.savetxt("hilbert_ranks.csv", hilbert_idxs, delimiter=",")
          
    analyze_ranking_performance(model_ranks, qp_matrix, "NEURAL_NET")
    analyze_ranking_performance(hilbert_ranks, qp_matrix, "HILBERT")
...
def load_iris_data_2d_norm():

    iris = load_iris() 
    data = iris.data 
    data_2d = data[:,1:3]
    max_vals = data_2d.max(axis=0)
    min_vals = data_2d.min(axis=0)
    data_2d_norm = (data_2d - min_vals) / (max_vals - min_vals + 1e-10)
    data_2d_norm = data_2d_norm * (N_POINTS_PER_AXIS-1)
    return data_2d_norm
